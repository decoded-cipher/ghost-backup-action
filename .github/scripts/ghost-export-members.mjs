...
const dateTag = process.env.BACKUP_DATE;
...
(async () => {
  const res = await fetch(`${adminBase}/members/csv/`, {
    method: "POST",
    headers: { Authorization: `Ghost ${token}` }
  });
  if (!res.ok) {
    const txt = await res.text().catch(() => "");
    throw new Error(`Members CSV export failed (${res.status}): ${txt.slice(0, 500)}`);
  }
  const buf = Buffer.from(await res.arrayBuffer());
  const file = path.join(outDir, `ghost-members-${dateTag}.csv`);
